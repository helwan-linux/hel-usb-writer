...
class HelwanUSBWriter(QMainWindow):
    # لا داعي لـ HelwanUSBWriter.current_language = "en" هنا، سيتم تعيينها لكل مثيل

    def __init__(self, parent=None):
        super().__init__(parent)
        self.current_language = "en"  # تعيين اللغة للمثيل هنا
    ...
    def browse_iso_file(self):
        # *** التعديل الرئيسي هنا: استخدام translations مباشرة بدلاً من self.translations ***
        # تأكد من أن `translations` (المستوردة من translations.py) هي التي تستخدمها
...
class SHA256Worker(QThread):
    ...
    def run(self):
        try:
            ...
            # استخدام HelwanUSBWriter.current_language هنا لأن هذا worker لا يمتلك self.current_language
...
if __name__ == "__main__":
    ...
    # هنا يمكن تحديد اللغة الافتراضية للتطبيق بأكمله إذا أردت
    # HelwanUSBWriter.current_language = "ar" 
    # لكن الكود في HelwanUSBWriter.__init__ سيتجاوز هذا لضمان تعيينها للمثيل
